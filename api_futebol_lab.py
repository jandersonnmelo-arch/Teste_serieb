...
def extract_players_from_detail(detail, lineups=None, events=None):
    ...
    # Fallback final: alguns formatos da API não possuem uma chave explícita
    # de escalação/jogadores. Como o detalhe já está persistido, podemos
    # varrer o JSON inteiro sem consumir novas chamadas da API.
    if not records:
        fallback = _discover_players_fallback(
            detail, home=home, away=away, inherited_team={}, result=[], seen=set()
        )
        records = _merge_player_records(records, fallback)

    return records



def _discover_players_fallback(value, home=None, away=None, inherited_team=None, result=None, seen=None):
    """Varredura de segurança para formatos de jogadores não padronizados.

    Algumas respostas da API Futebol podem não expor a escalação sob uma chave
    previsível. Neste caso, percorremos o JSON já carregado procurando objetos
    que tenham identidade clara de jogador. Isto NÃO faz chamada HTTP.
    """
    if result is None:
        result = []
    if seen is None:
    ...
    # Propaga explicitamente o contexto mandante/visitante.
    for key, child in value.items():
        ...
        if nk in {
            ...
            _discover_players_fallback(child, home, away, home, result, seen)
        elif nk in {
            "visitante", "fora", "away", "away_team", "time_visitante",
            "time_fora", "visitante_time", "away_team_players",
        }:
            _discover_players_fallback(child, home, away, away, result, seen)

    name = _player_name_from_obj(value)
    pid = _player_id_from_obj(value)
    player_stats = _stats_from_player_obj(value)
    player_shape_keys = {
        "numero", "camisa", "number", "posicao", "position", "pos",
        "statistics", "estatisticas", "stats", "minutos", "minutes",
        "titular", "starter", "substitute", "substituto",
    }

    if name and (pid is not None or any(k in value for k in player_shape_keys)):
        team = current_team or _team_context(player_stats)
        if not _valid_team_ref(team):
            team = {}
        stable_id = _stable_player_id(value, team)
        if stable_id is not None and str(stable_id) not in seen:
            result.append({
                "player": value,
                "team": team,
                "stats": player_stats,
    ...
    # Continua a busca por todos os filhos, inclusive wrappers desconhecidos.
    for key, child in value.items():
        if key in {"statistics", "estatisticas", "stats", "estatistica", "dados_jogador"}:
            continue
        if isinstance(child, (dict, list)):
            _discover_players_fallback(
                child, home, away, current_team, result, seen
...
def _merge_player_records(primary, fallback):
    """Mescla registros sem duplicar atletas já identificados."""
    result = list(primary or [])
    seen = set()
    for record in result:
        player = record.get("player") or {}
        team = record.get("team") or {}
        sid = _stable_player_id(player, team)
        if sid is not None:
            seen.add(str(sid))

    for record in fallback or []:
        player = record.get("player") or {}
        team = record.get("team") or {}
        sid = _stable_player_id(player, team)
