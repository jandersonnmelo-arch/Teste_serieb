...
def get_token():
    ...
    return (
        get_secret("API_FUTEBOL_TOKEN")
        or get_secret("API_FUTEBOL_KEY")
        or get_secret("CHAVE_API_FUTEBOL")
    )
...
def api_get(path, cache):
    ...
    try:
        ...
        # Toda resposta HTTP recebida conta como chamada da API.
        ...
        if response.status_code == 403:
            error = (
                "HTTP 403 — acesso negado ou recurso não liberado pelo plano."
            )
            cache["last_error"] = error
            return None, error
...
def discover_serie_b(cache, force=False):
    ...
    data, error = api_get("/campeonatos", cache)
