...
def extract_named_list(payload, names):
    """Localiza somente o bloco solicitado, sem misturar jogadores/eventos/escalações.

    A versão anterior adicionava aliases de vários blocos ao mesmo conjunto.
    Isso podia fazer uma busca por ``escalacoes`` retornar ``jogadores`` ou
    ``eventos`` antes de chegar ao bloco correto. Aqui cada chamada respeita
    exclusivamente os aliases recebidos.
    """
    ...
    # Último recurso: busca recursiva, mas somente pelas chaves solicitadas.
...
def _collect_player_entries(value, inherited_team=None, result=None, seen=None):
    ...
    current_team = _team_context(value) or inherited_team or {}
...
def extract_players_from_detail(detail, lineups=None, events=None):
    ...
    Ordem de prioridade:
      1. bloco direto de jogadores/atletas;
      2. escalações/titulares/reservas;
      3. jogadores identificados nos eventos, somente como fallback.
    """
    ...
    if not records and lineups:
        records = _collect_player_entries(lineups, result=[], seen=seen)

    if events:
        records.extend(_players_from_events(events, seen=seen))
...
def _extract_player_records(enriched):
    """Normaliza qualquer formato de jogadores da camada enriquecida.

    Aceita tanto:
      - registros planos: {"player": {...}, "team": {...}, "stats": {...}}
      - grupos: {"team": {...}, "players": [...]}
      - formatos antigos com jogador/jogadores/atletas.
    """
    ...
    def add_record(player, team, stats):
        if isinstance(player, str):
            player = {"nome": player.strip()}
        if not isinstance(player, dict):
            return
    ...
    def visit(item, inherited_team=None):
        if isinstance(item, list):
            for child in item:
                visit(child, inherited_team)
            return

        if not isinstance(item, dict):
            return

        team = item.get("team") or item.get("time") or item.get("equipe") or item.get("clube") or inherited_team or {}
        explicit_player = item.get("player") or item.get("jogador") or item.get("atleta")
        stats = item.get("stats") or item.get("statistics") or item.get("estatisticas") or {}
        if isinstance(stats, list):
            stats = stats[0] if stats and isinstance(stats[0], dict) else {}

        # Registro plano de _collect_player_entries.
        if explicit_player is not None:
            add_record(explicit_player, team, stats)
            return

        # O próprio objeto pode ser o jogador.
        name = _player_name_from_obj(item)
        pid = _player_id_from_obj(item)
        if name is not None and (
            pid is not None
            or any(k in item for k in (
                "numero", "camisa", "number", "posicao", "position", "pos",
                "statistics", "estatisticas", "stats"
            ))
        ):
            add_record(item, team, _stats_from_player_obj(item))
            return

        # Grupo de jogadores.
        for key in ("players", "jogadores", "atletas", "titulares", "starters",
                    "reservas", "substitutes", "substitutos"):
            child = item.get(key)
            if child is not None:
                visit(child, team)
...
def update_team_history(cache, enriched):
    ...
    # Primeiro usa a camada estável já enriquecida. Se ela estiver vazia,
    # tenta recuperar os jogadores diretamente do RAW que já foi persistido.
    # Isso corrige bases antigas sem fazer nenhuma chamada à API.
    player_records = _extract_player_records(enriched)

    if not player_records:
        raw_detail = enriched.get("raw")
        if isinstance(raw_detail, (dict, list)):
            recovered_players = extract_players_from_detail(
                raw_detail,
                extract_named_list(raw_detail, ["escalacoes", "escalações", "lineups", "lineup"]),
                extract_named_list(raw_detail, ["eventos", "events"]),
            )
            if recovered_players:
                enriched["jogadores"] = recovered_players
                player_records = _extract_player_records(enriched)
...
if st.button(
    "🛠️ Recuperar jogadores do enriquecimento existente",
    use_container_width=True,
):
    before_calls = int(cache.get("api_calls", 0))
    ...
    ok, save_error = persist_cache(
        cache,
        "Laboratório: recuperou jogadores do enriquecimento existente",
    )
    ...
    if not ok:
        ...
        st.session_state["lab_cache"] = cache
        st.success(
            "✅ Recuperação concluída usando somente os dados já persistidos. "
            f"Nenhuma chamada à API foi necessária ({before_calls} → {after_calls})."
        )
        st.write(
...
st.caption(
    "Use este botão para corrigir uma partida já enriquecida quando os jogadores "
    "ainda não tiverem sido indexados. O RAW persistido é reutilizado."
)
