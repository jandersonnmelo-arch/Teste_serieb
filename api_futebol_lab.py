...
def extract_named_list(payload, names):
    ...
    if isinstance(value, list):
        return value
    if isinstance(value, dict):
        # Alguns retornos agrupam o bloco por mandante/visitante ou
        # diretamente dentro de um objeto. Mantemos o objeto para que
        # os parsers especializados possam percorrê-lo.
        return value
    return []
...
def _player_id_from_obj(value):
    ...
    return first_value(value, [
        "jogador_id", "player_id", "id_jogador", "id",
    ])
...
def _player_name_from_obj(value):
    if not isinstance(value, dict):
        return None
    return first_value(value, [
        "nome", "name", "nome_jogador", "nome_popular",
    ])
...
def _collect_player_entries(value, inherited_team=None, result=None, seen=None):
    """Percorre escalações/jogadores em profundidade sem fazer API.

    A API pode entregar jogadores como:
      - {jogador: {...}, estatisticas: {...}}
      - {player: {...}, statistics: [...]}
      - {jogadores: [...]} dentro de cada time
      - titulares/reservas dentro de escalações
      - estruturas aninhadas em mandante/visitante.
    """
    if result is None:
        result = []
    if seen is None:
    ...
    # Caso explícito: o objeto representa um jogador.
    explicit_player = first_value(value, ["jogador", "player", "atleta"])
    if isinstance(explicit_player, dict):
        player_id = _player_id_from_obj(explicit_player)
        if player_id is not None:
            stats = _stats_from_player_obj(value) or _stats_from_player_obj(explicit_player)
            result.append({
    ...
    # Caso direto: o próprio objeto tem identidade de jogador.
    direct_id = _player_id_from_obj(value)
    direct_name = _player_name_from_obj(value)
    looks_like_player = (
        direct_id is not None
        and direct_name is not None
        and not any(k in value for k in ("statistics", "estatisticas", "stats"))
    )
    if looks_like_player and str(direct_id) not in seen:
        result.append({
            "player": value,
            "team": current_team,
            "stats": _stats_from_player_obj(value),
        })
        seen.add(str(direct_id))

    # Percorre primeiro os containers conhecidos.
    for key in (
        "jogadores", "players", "atletas", "titulares", "starters",
        "reservas", "substitutes", "substitutos", "escalacao", "lineup",
        "lineups", "mandante", "visitante", "casa", "fora",
    ...
    # Fallback recursivo para formatos não previstos.
    known = {
        "jogador", "player", "atleta", "team", "time", "equipe",
        "clube", "statistics", "estatisticas", "stats", "estatistica",
    }
...
def extract_players_from_detail(detail, lineups=None):
    """Extrai jogadores do detalhe já armazenado, priorizando o bloco direto
    e usando escalações/titulares/reservas como fallback. Nenhuma API é chamada."""
    direct = extract_named_list(detail, ["jogadores", "players", "atletas"])
    records = []
    seen = set()
...
def enrich_match_detail(match_id, detail):
    ...
    players = extract_players_from_detail(detail, lineups)
