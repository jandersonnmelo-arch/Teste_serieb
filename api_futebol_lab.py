...
def update_team_history(cache, enriched):
    ...
    for side in ("casa", "fora"):
        ...
        if enriched["partida_id"] not in [str(x.get("partida_id")) for x in bucket["jogos"]]:
            bucket["jogos"].append({
                "partida_id": enriched["partida_id"],
                "mandante": side == "casa",
                "adversario": enriched.get("times", {}).get("fora" if side == "casa" else "casa"),
                "placar": enriched.get("placar", {}),
                "estatisticas": enriched.get("estatisticas", []),
                "eventos": enriched.get("eventos", []),
