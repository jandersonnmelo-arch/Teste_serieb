    ...
    if isinstance(data, dict):
        for key in keys:
            ...
            if isinstance(value, list):
                return value
        numeric_values = [value for key, value in data.items() if str(key).isdigit() and isinstance(value, dict)]
        if numeric_values:
...
st.caption("A API retornou os campeonatos como objetos indexados por chaves numéricas; o laboratório reconhece esse formato automaticamente.")
...
if comps:
    for c in comps:
        text = " ".join(str(c.get(k) or "") for k in ["nome", "name", "slug", "descricao", "description"]).lower()
        if "série b" in text or "serie b" in text:
            ...
            serie_b_ids.append((str(campeonato_id) if campeonato_id is not None else "", c.get("nome") or c.get("name"), str(edicao_id) if edicao_id is not None else ""))
...
manual_championship_id = st.text_input("ID do campeonato Série B (se necessário)", value=serie_b_ids[0][0] if serie_b_ids else "")
...
if st.button("📚 Carregar fases"):
    ...
    if not cid:
        ...
        if error:
            ...
            if phases:
                st.dataframe([
                    {"ID": p.get("id") or p.get("fase_id"), "Nome": p.get("nome") or p.get("name"), "Status": p.get("status")}
                    for p in phases
                ], use_container_width=True, hide_index=True)
...
phase_id = st.text_input("ID da fase", value=str(phases[0].get("fase_id") or phases[0].get("id")) if phases else "")
